...
> ## ★ 当前主链路（重点）：48627 组合认证
>
> 本仓库目前**主要关注 48627 端口**的「Self-signed Application + X.509 User 组合认证」链路：
>
> * **配置文件**：`config_self_signed_x509_user.yaml`（端口 48627，固定 `Basic256Sha256 + SignAndEncrypt`，只开放 X.509 User 认证）
> * **启动**：`.venv\Scripts\python.exe main.py config_self_signed_x509_user.yaml`（**必须用 `.venv`**，全局 `D:\Python311` 的 asyncua 是 1.1.8，会被版本断言拒绝）
> * **PKI**：`test_material\self_signed\`（Application trust store 已授信产品证书 `datahub-default-channel.der`，见 `0922\`）
> * **测试**：`tests\self_signed_x509_user_tests.py`，最新结果见 `48627_test_result_20260922.txt`（9/9 PASS）
> * **链路说明**：两道独立信任关卡——Application 证书在 CreateSession 由 trust store 校验；User 证书在 ActivateSession 由 `user_auth.x509_user_cert` DER 白名单校验；两证严格隔离。
>
> 其余历史场景（48620/48621/48625/48626 的配置、测试与 CA 证书）已归档到仓库根目录 `archive\hda_509_other_scenarios\`，需要时从那里恢复即可。
