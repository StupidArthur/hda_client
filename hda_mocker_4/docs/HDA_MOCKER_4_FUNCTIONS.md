...
Mocker 可在配置指定的独立端口提供轻量 HTTP 诊断协议。简版接口返回总体状态和真实业务摘要；详细接口同时提供业务信息与诊断数据。可观测项包括 DA/HDA 最近提交、期望与实际位号数、原生/实时 HDA 时间范围、TCP 连接数、UA Session 数，以及客户端地址和 Application/Session 元数据。接口字段均为标量平铺 map，方便不同语言的管理工具兼容接入。

## 12. 性能评估

### v1.4.0 CLI 与日志

启动时使用 `--config` 指定配置文件。标准运行日志同时写入控制台与 EXE 同目录的 `logs/hda_mocker_4.log`，并按大小滚动；Go 运行时崩溃堆栈另写入 `logs/hda_mocker_4_crash.log`。诊断接口提供当前状态，文件日志保留历史原因。

v1.4.1 修复订阅生命周期；v1.4.2 在此基础上限制慢客户端的通知积压，并在诊断中暴露订阅、监控项及通知合并指标。详见 [版本说明](HDA_MOCKER_4_RELEASE_1_4_1_1_4_2.md)。
