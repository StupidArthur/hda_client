...
func (s *SessionService) CreateSession(sc *uasc.SecureChannel, r ua.Request, reqID uint32) (ua.Response, error) {
	...
	if len(matching_endpoints) == 0 && req.EndpointURL != "" {
		// A wildcard host is valid for listening but cannot be advertised to a
		// remote client. Return equivalent endpoints using the URL by which this
		...
		for _, ep := range s.srv.endpoints {
			...
			copy := *ep
			copy.EndpointURL = req.EndpointURL
			if ep.Server != nil {
				app := *ep.Server
				app.DiscoveryURLs = []string{req.EndpointURL}
				copy.Server = &app
			}
			matching_endpoints = append(matching_endpoints, &copy)
		}
	}
